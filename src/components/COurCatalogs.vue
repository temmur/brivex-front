<template>
  <section id="catalog" class="w-full overflow-hidden bg-[#FAFAF9] py-14 sm:py-16 lg:py-24 xl:py-28 scroll-mt-24">
    <div class="container mx-auto max-w-7xl max-lg:px-4">
      <div class="mb-10 flex flex-col items-center text-center sm:mb-14 lg:mb-16">
        ...
        <h2 class="flex flex-wrap justify-center gap-x-2 text-3xl font-black uppercase tracking-tight sm:gap-x-3 sm:text-4xl md:text-5xl xl:text-6xl">
          <span class="text-[#111828]">{{ t('catalog.title_part1') }}</span>
          <span class="text-[#00A740]">{{ t('catalog.title_part2') }}</span>
        </h2>
      ...
      <div class="relative">
        ...
        <div
          ref="carouselRef"
          class="no-scrollbar flex touch-pan-x items-stretch snap-x snap-mandatory gap-6 overflow-x-auto scroll-smooth px-2 pt-12 pb-12 sm:gap-7 xl:gap-8"
        ...
        >
          <article
            ...
            class="group relative flex h-auto min-h-[520px] w-[86%] shrink-0 snap-start flex-col items-center rounded-[28px] bg-white p-6 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.03)] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-3 hover:shadow-[0_40px_80px_-20px_rgba(22,163,74,0.25)] sm:min-h-[550px] sm:w-[calc((100%-1.75rem)/2)] sm:rounded-[34px] sm:p-8 lg:w-[calc((100%-3.5rem)/3)] xl:min-h-[590px] xl:w-[calc((100%-6rem)/4)] xl:rounded-[40px] xl:p-10"
          >
            ...
            <div class="relative z-10 mb-6 sm:mb-8 xl:mb-10">
              ...
              <div class="h-32 w-32 overflow-hidden rounded-full border-[6px] border-white bg-white shadow-2xl transition-transform duration-700 group-hover:scale-110 group-hover:-rotate-2 sm:h-40 sm:w-40 sm:border-[7px] xl:h-48 xl:w-48 xl:border-[8px]">
                <img
              ...
              <div class="absolute -bottom-4 left-1/2 h-4 w-24 -translate-x-1/2 rounded-[100%] bg-black/5 blur-md transition-all duration-500 group-hover:bg-black/20 group-hover:blur-xl"></div>
            </div>

            <div class="relative z-10 flex w-full flex-grow flex-col items-center text-center">
              ...
              <div class="mb-6 w-full space-y-2 border-t border-gray-100 pt-4 transition-colors duration-500 group-hover:border-white/10 sm:mb-8 sm:pt-5 xl:mb-10 xl:pt-6">
                <p v-for="(line, idx) in getProductDetails(product)" :key="idx" class="text-xs font-bold uppercase tracking-wider text-gray-400 transition-colors duration-500 group-hover:text-green-100">
                  {{ line }}
                </p>
              </div>
        ...
        <div class="mt-5 flex items-center justify-center gap-4 lg:hidden">
          <button type="button" class="flex h-10 w-10 items-center justify-center rounded-full border border-green-200 bg-white text-green-700 shadow-sm transition hover:bg-green-600 hover:text-white disabled:opacity-40" :disabled="!canScrollPrev" @click="scrollPrev">
          ...
          <button type="button" class="flex h-10 w-10 items-center justify-center rounded-full border border-green-200 bg-white text-green-700 shadow-sm transition hover:bg-green-600 hover:text-white disabled:opacity-40" :disabled="!canScrollNext" @click="scrollNext">
      ...
      <div class="mt-10 flex justify-center">
        <router-link to="/products" class="group inline-flex items-center gap-3 rounded-full border border-green-600 bg-white px-7 py-3 text-xs font-black uppercase tracking-[0.2em] text-green-700 transition-all duration-300 hover:-translate-y-0.5 hover:bg-green-600 hover:text-white">
          <span>{{ t('catalog.see_all') }}</span>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
        </router-link>
      </div>
    </div>
  </section>
...
import { getProductContent, productsData, type ProductDataItem } from '@/data/productsData'
